...
param = 9 #number of parameters
M = 4 #interface factor
Nr = 2 #resumpling
fmax = maximum(freq)
Ns = 2*M*fmax + 1 #sample number
...
for k in 1:2
    ...
    for ns in 1:Ns
        ...
        y1 = sum(sol[end][:, 2:end-1, 5] .- sol[end][:, 3:end, 5])/(Ly*(Lx-2)) #sensitivity function 1
        ...
        y2 = argmin(abs.(Nc))[1] #sensitivity function 2
        for mf in 1:M*fmax
