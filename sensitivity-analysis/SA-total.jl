...
begin
    A_t1 = zeros(M*fmax)
    B_t1 = zeros(M*fmax)
    A_i1 = zeros(param, floor(Int, fmax/2)) 
    B_i1 = zeros(param, floor(Int, fmax/2)) 
    V_i1 = zeros(param)
    V_t1 = 0.0

    A_t2 = zeros(M*fmax)
    B_t2 = zeros(M*fmax)
    A_i2 = zeros(param, floor(Int, fmax/2)) 
    B_i2 = zeros(param, floor(Int, fmax/2)) 
    V_i2 = zeros(param)
    V_t2 = 0.0
end
...
for k in 1:2
    ...
    for pa in 1:param
        for ns in 1:Ns
            ...
            y2 = argmin(abs.(Nc))[1]
            for mf in 1:M*fmax
                A_t1[mf] = A_t1[mf] + (1/Ns)*(y1*cos(mf*s))
                B_t1[mf] = B_t1[mf] + (1/Ns)*(y1*sin(mf*s))
                A_t2[mf] = A_t2[mf] + (1/Ns)*(y2*cos(mf*s))
                B_t2[mf] = B_t2[mf] + (1/Ns)*(y2*sin(mf*s))
            end
            for f in 1:floor(Int, fmax/2)
                A_i1[pa, f] = A_i1[pa, f] + (1/Ns)*(y1*cos(f*s))
                B_i1[pa, f] = B_i1[pa, f] + (1/Ns)*(y1*sin(f*s))
                A_i2[pa, f] = A_i2[pa, f] + (1/Ns)*(y2*cos(f*s))
                B_i2[pa, f] = B_i2[pa, f] + (1/Ns)*(y2*sin(f*s))
            end
            print("k=", k, " parameter=", pa, "Ns=", ns)
        end
        V_i1[pa] = V_i1[pa] + 2*sum((A_i1[pa, :].^2) .+ (B_i1[pa, :].^2))
        V_i2[pa] = V_i2[pa] + 2*sum((A_i2[pa, :].^2) .+ (B_i2[pa, :].^2))
