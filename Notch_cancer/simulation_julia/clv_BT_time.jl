...
@time bt_sol = solve(prob_bt, Heun(), progress = true, progress_steps = 1.0, callback=TerminateSteadyState(1e-3), saveat=[T_end]);
# BS3, Tsit5, Heun, 
bt_sol.t
...
begin
    ...
    p_ut = (
            ...
            β1=0.1, β21=5.0, β22=8.0, β3=2.0, β41=8.0, β42=8.0,
            ν0=0.5, ν1=5.0, ν2=25.0, ν3=5.0, ωN=0.01, ωD=0.01,
            p1=0.0, p23=0.0, pβ=0.0, canc=canc, h_max=h_max,
            cache...
    )
...
@time bt_sol = solve(prob_cbt, Heun(), progress=true, progress_steps=1.0, callback=TerminateSteadyState(1e-3), saveat=[T_end]);
Hes =  bt_sol[:, :, end, end] .- Array(u0[:, :, end])

time_05 = zeros(8)
time_08 = zeros(8)
count_05 = zeros(8)
count_08 = zeros(8)
time_05[1] = bt_sol.t[1]
time_08[1] = bt_sol.t[1]
    ...
    for treat = 1:7
        for effect = 1:2
            p = (
                ...
                β1=0.1, β21=5.0, β22=8.0, β3=2.0, β41=8.0, β42=8.0,
                ν0=0.5, ν1=5.0, ν2=25.0, ν3=5.0, ωN=0.01, ωD=0.01,
                p1=p1[effect, treat], p23=p23[effect, treat], pβ=pβ[effect, treat],
                canc=canc, h_max=h_max, cache...
            )
            cprob_bt = ODEProblem(brain_func!, u0, tspan, p)
            @time @views bt_sol = solve(cprob_bt, Heun(), progress=true, progress_steps=1.0, saveat=[T_end], callback=TerminateSteadyState(1e-3));
            Hes =  bt_sol[:, :, end, end] .- Array(u0[:, :, end])

            #ge the time when steady state is reached--------------------------------
            if effect == 1
                time_05[treat+1] = bt_sol.t[1]
                count_05[treat+1] = count(Hes .> 0.75*h_max)
            end
            if effect == 2
                time_08[treat+1] = bt_sol.t[1]
...
label = ["untreated", L"$p_1$", L"$p_{23}$", L"$p_{\beta}$",
        L"$p_1+p_{23}$", L"$p_1+p_{\beta}$", L"$p_{23}+p_{\beta}$",
        L"$p_1+p_{23}+p_{\beta}$"] 
x = Array(1:8) 
width = 0.4 
...
fig1.savefig("clv_spr-time_BT.png") 
fig1
...
fig2.savefig("clv_spr-count_BT.png")
fig2
