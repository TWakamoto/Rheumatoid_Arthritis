...
cb2 = TerminateSteadyState(5e-3) #if all cells reach steady state, terminate the simulation
...
while maximum(Array(u0[2, 2:yoko+1, end]))<h_max*0.75 && maximum(Array(u0[tate+1, 2:yoko+1, end]))<h_max*0.75 && maximum(Array(u0[2:tate+1, 2, end]))<h_max*0.75 && maximum(Array(u0[2:tate+1, yoko+1, end]))<h_max*0.75
    ...
    heatmap(Array(u0[2:tate+1, 2:yoko+1, end]), title="t=$(round(bt_sol.t[end], digits=1))", c=:jet1, clim=(0, h_max), aspect_ratio=1)
    frame(anim)

    #updata
    T_end = bt_sol.t[end] + 50000
    tspan = (bt_sol.t[end], T_end)
    prob_cbt = ODEProblem(brain_func!, u0, tspan, p_ut)
    cb1 = FunctionCallingCallback(plot_heatmap, funcat = LinRange(bt_sol.t[end], T_end, 30)) #create image at regular intervals
    cbs = CallbackSet(cb1, cb2)
    clean_up_GPU()
end

clean_up_GPU()
gif(anim, "CellDivision_BT.gif", fps=90) #create gif animation
heatmap(Array(bt_sol[end][2:tate+1, 2:yoko+1, 11]), c=:jet1, clim=(h_max*0.5, h_max), aspect_ratio=1)
...
gif(anim, "CellDivision_clv_BT.gif", fps=90)
clean_up_GPU()