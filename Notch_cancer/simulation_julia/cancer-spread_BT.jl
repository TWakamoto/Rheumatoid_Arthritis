include("/Users/tamaki/Library/CloudStorage/OneDrive-個人用/Notch signaling/シミュレーション/function/utils.jl")
include("/Users/tamaki/Library/CloudStorage/OneDrive-個人用/Notch signaling/シミュレーション/function/initial_condition.jl")
...
@time bt_sol = solve(prob_bt, BS5(), progress = true, progress_steps = 1.0, saveat = [T_end]);
# BS3, Tsit5, Heun, 
...
begin
    #make initial data-----------------------------------------------------
    maxs = (
        Nm1_max = bt_sol[end].Nm1[index_max],
        Nm2_max = bt_sol[end].Nm2[index_max],
        Dm1_max = bt_sol[end].Dm1[index_max],
        Dm2_max = bt_sol[end].Dm2[index_max],
        Nc1_max = bt_sol[end].Nc1[index_max],
        Nc2_max = bt_sol[end].Nc2[index_max],
        Dc1_max = bt_sol[end].Dc1[index_max],
        Dc2_max = bt_sol[end].Dc2[index_max],
        I1_max  = bt_sol[end].I1[index_max],
        I2_max  = bt_sol[end].I2[index_max],
        H_max   = bt_sol[end].H[index_max],
    )
    ...
    prob_cbt = ODEProblem(brain_func!, u0_canc, tspan, p_ut)
end

@time cbt_sol = solve(prob_cbt, BS5(), progress = true, progress_steps = 1.0);

#-----------------------get # of cells with high expression of HES-1-----------------------
hes =  cbt_sol[end].H[2:end-1, 2:end-1] .- u0_canc.H[2:end-1, 2:end-1]
...
time = Observable(1)
data = @lift cbt_sol[$time].H[2:tate+1, 2:yoko+1]
fig, ax, hm = heatmap(data, axis = (title = @lift("t= $(round(cbt_sol.t[$time], digits=1))"),))
Colorbar(fig[:, end+1], hm)
len_bt = size(cbt_sol)[2]
framerate = Int(floor(len_bt/10))
timestamps = range(1, len_bt)

Makie.record(fig, "HES-1_BT.mp4", timestamps;
...
begin
    ...
    for treat = 1:7
        for patient = 1:10
            ...
            cprob_bt = ODEProblem(brain_func!, u0_canc, tspan, p)
            @time cbt_sol = solve(cprob_bt, BS5(), progress = true, progress_steps = 1.0);
...
fig1.savefig("cancer_spr_BT_paper.png")
...
fig2
fig2.savefig("cancer_spr_BT.png")