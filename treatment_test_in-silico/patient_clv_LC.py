...
lccell5 = np.zeros((m, 7))
lccell75 = np.zeros((m, 7))
...
# %%=========================================
p_1 = np.zeros((m, ver, wid, 7))
p_23 = np.zeros((m, ver, wid, 7))
p_b = np.zeros((m, ver, wid, 7))
r_1 = np.random.rand(m, ver, wid)
r_23 = np.random.rand(m, ver, wid)
...
for i in range(m):
    for j in range(7):
        lccell5[i, j] = 100*(np.count_nonzero(H1[i, :, :, j]>0.5))/(ver*wid)
        lccell75[i, j] = 100*(np.count_nonzero(H1[i, :, :, j]>0.75))/(ver*wid)
lccell5 = lccell5/hst05
lccell75 = lccell75/hst075
...
for j in range(1, 8):
    ave05[j] = np.mean(lccell5[:, j-1])
    ave075[j] = np.mean(lccell75[:, j-1])
    SD05[j] = np.std(lccell5[:, j-1])
    SD075[j] = np.std(lccell75[:, j-1])
...
fig1.savefig("patient_inh_ave-SD_LC_p(2).png")
...
fig2.savefig("patient_inh_ave-SD_LC(2).png")
