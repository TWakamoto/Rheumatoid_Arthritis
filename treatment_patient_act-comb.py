...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 13 10:09:37 2023
...
#p_a = np.random.rand(m, ver+2, wid+2)
#p_a[0, :, :] = np.zeros((ver+2, wid+2))
#p_b = np.random.rand(m, ver+2, wid+2)
#p_b[0, :, :] = np.zeros((ver+2, wid+2))
#p_1 = 0.0
p_a = 0.0
p_b = 0.0
#function
...
def Notchm1(x, d_ave1, d_ave2, nc1):
    return (1+p_a)*(-alpha1*d_ave1*x - alpha2*d_ave2*x) - mu1*x + (1+p_1)*gamma1*nc1
def Notchm2(x, d_ave1, d_ave2, nc2):
    return (1+p_a)*(-alpha5*d_ave1*x - alpha6*d_ave2*x) - mu1*x + (1+p_1)*gamma1*nc2
