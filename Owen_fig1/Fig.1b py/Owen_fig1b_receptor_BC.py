...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 19 15:25:42 2021
...
#配列の準備
t = np.zeros(N)
a = np.full((Iy+2, Ix+2), 3000) #the number of ligand
b = np.full((Iy+2, Ix+2), 1000) #the number of bound receptor-ligand complex
f = np.full((Iy+2, Ix+2), 5000) #the number of free receptor
...
for l in range(N-1):
    ...
    f[:, 0] = f[:, -1]
    f[:, Iy+1] = f[:, 1]
    aext = (a[1:-1, 2:] + a[1:-1, :-2] + a[2:, :] + a[:-2, :])/4
    bext = (b[1:-1, 2:] + b[1:-1, :-2] + b[2:, :] + b[:-2, :])/4
    fext = (f[1:-1, 2:] + f[1:-1, :-2] + f[2:, :] + f[:-2, :])/4
...
anim.save('Fig1b_receptor_BC.gif', writer="pillow")
plt.show()