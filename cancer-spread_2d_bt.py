...
class Model():
    ...
    def __call__(self, t, y):
        ...
        if( canc > 0 ):
            ...
            index = torch.where(H1[2:-2, 2:-2] > 0.65*self.hmax)
            index_up = index[0]+1
...
T = 250.0 #最終時
model = Model(n_grid=1000, e = 0.01, device=device)
...
p_0 = 0.0
# %%==============================================================
model.canc = 332  # number of cancer
...
plot_animation(t_anim, sol_H1anim[:, 1:-1, 1:-1], "cancer-spread_2d_h_bt.gif")

# %%===========================================================================
# get color map of protein (or gene) A (animation)
sol_anim = sol[:, 10, 1:-1, 1:-1].to('cpu').detach().numpy().copy()
plot_animation(t_anim, sol_anim, "cancer-spread_2d_h_bt.gif")
...
plot_graph(t_anim, sol_[:, 10, 1:-1, 1:-1], "test_graph_h_bt.gif", 20)

# %%==========================================================================================================================================
count = np.zeros(11)
hmax_int = int(np.max(sol_H1anim[-1, 1:-1, 1:-1]))
itvl = int(hmax_int/10)
for i in range(0, hmax_int+1, itvl):
    count[i] = (np.count_nonzero((sol_H1anim[-1, 1:-1, 1:-1]>=(i*5)) & (sol_H1anim[-1,1:-1,1:-1] < (i+1)*5)))/(ver*wid)
