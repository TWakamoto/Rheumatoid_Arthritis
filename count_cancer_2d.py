...
Dm1[1:-1, 1:-1] = y_init[1, :, :]
Nc1[1:-1, 1:-1] = y_init[2, :, :]
Dc1[1:-1, 1:-1] = y_init[3, :, :]
I1[1:-1, 1:-1] = y_init[4, :, :]
Nm2[1:-1, 1:-1] = y_init[5, :, :]
Dm2[1:-1, 1:-1] = y_init[6, :, :]
Nc2[1:-1, 1:-1] = y_init[7, :, :]
Dc2[1:-1, 1:-1] = y_init[8, :, :]
I2[1:-1, 1:-1] = y_init[9, :, :]
...
for l in trange(N-1):
    ...
    Nm1[1:-1, 1:-1] = Nm1_new
    Nm2[1:-1, 1:-1] = Nm2_new
hmax = np.max(H1[1:-1, 1:-1]-y_init[-1, :, :])
h = (H1[1:-1, 1:-1]-y_init[-1, :, :])/hmax
std_cell_05 = 100*(np.count_nonzero(h>0.5))/(ver*wid)
std_cell_75 = 100*(np.count_nonzero(h>0.75))/(ver*wid)
...
for q in trange(m):
    ...
    for l in range(N-1):
        ...
        Nm2[1:-1, 1:-1] = Nm2_new
    h = (H1[1:-1, 1:-1]-y_init[-1, :, :])/hmax
    lccell5[q]  = 100*(np.count_nonzero(h>0.5))/(ver*wid)
    lccell75[q] = 100*(np.count_nonzero(h>0.75))/(ver*wid)

# %%
lccell5  = lccell5/std_cell_05
lccell75 = lccell75/std_cell_75
print("lung", lccell5)
print(lccell75)
print(gc.collect())
...
for q in trange(m):
    ...
    for l in range(N-1):
        ...
        Nm2[1:-1, 1:-1] = Nm2_new
    h = (H1[1:-1, 1:-1]-y_init[-1, :, :])/hmax
    btcell5[q]  = 100*(np.count_nonzero(h>0.5))/(ver*wid)
    btcell75[q] = 100*(np.count_nonzero(h>0.75))/(ver*wid)
print(btcell5)
print(btcell75)

# %%
btcell5  = btcell5/std_cell_05
btcell75 = btcell75/std_cell_75
print("brain", btcell5)
print(btcell75)
print(gc.collect())
...
ax1.set_xlabel(r"$\mu_{4}$", fontsize=15)
