...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun  8 13:49:35 2023
...
for j in range(2):
    ...
    for l in trange(N-1):
        n1_ave[:, :, 1:-1, 1:-1] = (Nm1[:, :, 1:-1, 0:-2] + Nm1[:, :, 1:-1, 2:])
        n2_ave[:, :, 1:-1, 1:-1] = (Nm2[:, :, 1:-1, 0:-2] + Nm2[:, :, 1:-1, 2:])
        d1_ave[:, :, 1:-1, 1:-1] = (Dm1[:, :, 1:-1, 0:-2] + Dm1[:, :, 1:-1, 2:])
        d2_ave[:, :, 1:-1, 1:-1] = (Dm2[:, :, 1:-1, 0:-2] + Dm2[:, :, 1:-1, 2:])
    #Delta in membrane
...
fig1.savefig("SA_total-effect_index_2cells_lc.png")
...
fig2.savefig("SA_total-effect_index_2cells_bt.png")
plt.show()
