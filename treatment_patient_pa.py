...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 10 13:19:46 2023
...
p_a = np.random.rand(m, ver, wid)
p_a[0, :, :] = np.zeros((ver, wid))
...
def Notchm1(x, d_ave1, d_ave2, nc1):
    return (1+p_a)*(-alpha1*d_ave1*x - alpha2*d_ave2*x) - mu1*x + gamma1*nc1
def Notchm2(x, d_ave1, d_ave2, nc2):
    return (1+p_a)*(-alpha5*d_ave1*x - alpha6*d_ave2*x) - mu1*x + gamma1*nc2
